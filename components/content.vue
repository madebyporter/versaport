<script setup lang="ts">
  ...
  const markdownToHtml = (markdown) => {
    ...
    html = html.replace(/<p>/g, '<p class="text-2xl leading-relaxed">');
    return html;
  };
...
<template>
  <div class="col-start-4 col-span-8 flex flex-col gap-10">
    <div 
      ...
      :key="content.id" 
      class="flex flex-col gap-10 border border-neutral-200 rounded-lg p-10"
    >
      ...
      <article class="flex flex-col gap-5">
        <h3 class="font-bold text-4xl">{{ content.header }}</h3>
