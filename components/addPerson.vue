<script setup>
  ...
  async function submitForm() {
    ...
    if (!urlPattern.test(formData.value.Portfolio)) {
      errorMessage.value = 'Invalid portfolio URL!';
      isSubmitting.value = false;
      return;
    }
    ...
    try {
      ...
      successMessage.value = 'Received your submission. I will review your submission within the next 7 days and contact you if approved!';
      formSubmitted.value = true;
    } catch (error) {
      console.log('Airtable error:', error);
      errorMessage.value = 'Submission failed. Something in your form is wrong or the form is broken.';
    }
    isSubmitting.value = false;
  }
</script>
...
<template>
  <div class="h-auto min-w-[260px] max-w-[560px] m-auto mb-10">
    <form v-if="!formSubmitted" @submit.prevent="submitForm" class="flex flex-col w-full gap-2 mb-4">
      <div class="mb-2">
      ...
      <FormInput v-model="formData.Marketing" type="checkbox" label="Subscribe to learn when new resources are launched!" />

      <div v-if="errorMessage" class="text-red-600 text-lg font-medium">
        {{ errorMessage }}
      </div>
    ...
    <div v-if="formSubmitted && successMessage" class="text-2xl font-medium">
      {{ successMessage }}
    </div>
  </div>
</template>
